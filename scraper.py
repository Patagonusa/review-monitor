...
class GoogleReviewsScraper:
    async def scrape_google_reviews(self, business_name, google_url):
        business_info = {
            "name": business_name, "url": google_url,
            "scraped_at": datetime.now().isoformat(),
            "overall_rating": None, "total_reviews": 0, "reviews": []
        }
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True)
            ctx = await browser.new_context(viewport={"width":1400,"height":900},
                user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/122",locale="en-US")
            page = await ctx.new_page()
            try:
                print(f"[SCRAPER] Loading: {business_name}")
                ...
                if m: business_info["total_reviews"] = int(m.group(1).replace(",",""))
                
                print(f"[SCRAPER] Rating: {business_info[\"overall_rating\"]}, Reviews: {business_info[\"total_reviews\"]}")
                
                # Click reviews tab
                for sel in ["button[aria-label*=Reviews]", "[role=tab]:has-text(Reviews)"]:
                    try:
                        btn = await page.query_selector(sel)
                        if btn: await btn.click(); await asyncio.sleep(2); break
                    except: pass
                ...
                # Extract reviews
                elems = await page.query_selector_all("div[data-review-id]") or await page.query_selector_all("div.jftiEf")
                print(f"[SCRAPER] Found {len(elems)} review elements")
                
                for el in elems[:50]:
                    try:
                        r = {"reviewer_name":None,"rating":None,"text":"","date":None,"owner_response":None}
                        n = await el.query_selector("div.d4r55")
                        if n: r["reviewer_name"] = (await n.inner_text()).split("\n")[0]
                        s = await el.query_selector("span.kvMYJc")
                        if s:
                            a = await s.get_attribute("aria-label")
                            if a and (m:=re.search(r"(\d)",a)): r["rating"]=int(m.group(1))
                        t = await el.query_selector("span.wiI7pd")
                        if t: r["text"] = await t.inner_text()
                        d = await el.query_selector("span.rsqaWe")
                        if d: r["date"] = await d.inner_text()
                        if r["reviewer_name"] or r["text"]: business_info["reviews"].append(r)
                    except: pass
                print(f"[SCRAPER] Extracted {len(business_info[\"reviews\"])} reviews")
        ...
        for biz in businesses:
            ...
            if url:
                ...
                try:
                    ...
                    results["businesses"].append({"id":biz.get("id"),"name":biz["name"],"error":str(e),"reviews":[]})
